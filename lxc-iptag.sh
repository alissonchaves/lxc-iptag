...
for command in apt-get pveversion systemctl; do
...
FILE_PATH="/opt/lxc-iptag/lxc-iptag"
if [[ -f "$FILE_PATH" ]]; then
  msg_info "Existing installation found at '$FILE_PATH'. Updating it."
  msg_ok "Existing installation found"
fi

msg_info "Installing Dependencies"
if ! apt-get update &>/dev/null; then
  msg_error "Failed to update APT package indexes"
  exit 1
fi
if ! apt-get install -y ipcalc net-tools &>/dev/null; then
...
if [[ ! -f /opt/lxc-iptag/lxc-iptag.conf ]]; then
  cat <<EOF >/opt/lxc-iptag/lxc-iptag.conf
...
CONFIG_FILE="/opt/lxc-iptag/lxc-iptag.conf"
STATE_DIR="/var/lib/lxc-iptag"
STATE_FILE="${STATE_DIR}/state"

mkdir -p "${STATE_DIR}"
chmod 0750 "${STATE_DIR}"

for command in ipcalc pct lxc-info ifconfig; do
  if ! command -v "${command}" >/dev/null 2>&1; then
    echo "Required command not found: ${command}" >&2
    exit 1
  fi
...
if [[ -f "${STATE_FILE}" ]]; then
  # This file is generated by this script and is owned by root.
  source "${STATE_FILE}"
fi
...
write_state() {
  ...
  umask 077
  {
    printf 'last_lxc_status=%q\n' "${last_lxc_status}"
    printf 'last_net_interface=%q\n' "${last_net_interface}"
    printf 'last_lxc_status_check_time=%q\n' "${last_lxc_status_check_time}"
    printf 'last_fw_net_interface_check_time=%q\n' "${last_fw_net_interface_check_time}"
    printf 'last_update_time=%q\n' "${last_update_time}"
  } >"${state_tmp}"
...
ip_in_cidr() {
  ...
  local ip_int netmask_int masked_ip_int broadcast

  ip_int=$(ip_to_int "${ip}")
  broadcast=$(ipcalc -b "${cidr}" 2>/dev/null | awk '/^Broadcast/ {print $2; exit}')
  [[ -n "${broadcast}" ]] || return 1
  netmask_int=$(ip_to_int "${broadcast}")
...
fw_net_interface_changed() {
  local current_net_interface
  current_net_interface=$(ifconfig | grep "^fw" || true)
...
update_lxc_iptags() {
  ...
  for vmid in ${vmid_list}; do
    ...
    # Get current IPs
    current_ips_full=$(lxc-info -n "${vmid}" -i 2>/dev/null | awk '{print $2}')
    ...
    if [[ "$(printf '%s\n' "${last_tagged_ips[@]}" | sort -u)" == "$(printf '%s\n' "${current_valid_ips[@]}" | sort -u)" ]]; then
...
if [[ -f "$SERVICE_FILE" ]]; then
  if ! grep -q '^Type=oneshot$' "$SERVICE_FILE" \
    || ! grep -q '^ExecStart=/opt/lxc-iptag/lxc-iptag$' "$SERVICE_FILE" \
...
TIMER_FILE="/lib/systemd/system/lxc-iptag.timer"
NEED_TIMER_UPDATE=false

if [[ -f "$TIMER_FILE" ]]; then
  if ! grep -q '^OnBootSec=1min$' "$TIMER_FILE" \
    || ! grep -q '^OnUnitActiveSec=300s$' "$TIMER_FILE"; then
    NEED_TIMER_UPDATE=true
  fi
else
  NEED_TIMER_UPDATE=true
fi

if [[ "$NEED_TIMER_UPDATE" == true ]]; then
  cat <<'EOF' >"$TIMER_FILE"
[Unit]
...
EOF
  ...
  msg_ok "systemd timer already correct"
fi
...
if ! systemctl enable -q --now lxc-iptag.timer &>/dev/null; then
  msg_error "Failed to enable or start lxc-iptag.timer"
  exit 1
fi
